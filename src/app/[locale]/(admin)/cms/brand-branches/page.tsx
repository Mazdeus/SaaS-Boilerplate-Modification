...
export default function BrandBranchesPage() {
  const [branches, setBranches] = useState<BrandBranch[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [editingId, setEditingId] = useState<number | null>(null);
  const [formData, setFormData] = useState<Partial<BrandBranch>>({
    isActive: true,
    order: 0,
  });
  ...
  const fetchBranches = async () => {
    try {
      const res = await fetch('/api/cms/brand-branches');
      const data = await res.json();
      if (data.success) {
        setBranches(data.data);
      }
    } catch (error) {
      console.error('Failed to fetch brand branches:', error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      console.error('Failed to create brand branch:', error);
    }
  };

  const handleEdit = (branch: BrandBranch) => {
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch(`/api/cms/brand-branches/${editingId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(formData),
      });
      ...
      console.error('Failed to update brand branch:', error);
    }
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const res = await fetch(`/api/cms/brand-branches/${id}`, {
        method: 'DELETE',
      });
      ...
      console.error('Failed to delete brand branch:', error);
    }
  };
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <CMSNavigation />
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900">Brand Branches</h1>
        <p className="text-gray-600">Manage brand locations and branch information</p>
      </div>
        ...
        <div className="mb-8 rounded-lg border border-gray-200 bg-white p-6">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div className="md:col-span-2">
              ...
              <input
                type="text"
                placeholder="e.g., Mon-Fri: 9:00 AM - 9:00 PM"
      ...
      <div className="space-y-4">
              ...
              <div className="space-y-4">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    {branch.subtitle && <p className="text-sm text-gray-500">{branch.subtitle}</p>}
        ...
        {branches.length === 0 && (
          <div className="rounded-lg border border-gray-200 bg-gray-50 p-8 text-center">
            <p className="text-gray-500">No brand branches found. Create your first branch above.</p>
          </div>
        )}
      </div>
    </div>
