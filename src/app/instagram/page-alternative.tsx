...
export default function InstagramPage() {
  ...
  const [juicerError, setJuicerError] = useState(false);

  useEffect(() => {
    // Load Juicer.io script
    const script = document.createElement('script');
    script.src = 'https://assets.juicer.io/embed.js';
    script.async = true;
    
    script.onload = () => {
      setIsJuicerLoaded(true);
      setJuicerError(false);
    };
    ...
    // Set a timeout to check if feed loaded
    const timeout = setTimeout(() => {
      const feedElement = document.querySelector('.juicer-feed');
    ...
    <>
      ...
      <main className="min-h-screen bg-gray-50">
        ...
        <div className="py-12 sm:py-16 lg:py-20">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            {/* Juicer.io Embed */}
            <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6 lg:p-8 min-h-[400px]">
              {juicerError ? (
                // Fallback: Direct Instagram Link
                <div className="text-center py-16">
                  ...
                  <h3 className="text-2xl font-bold text-gray-900 mb-4">
                    Lihat Aktivitas Kami di Instagram
                  </h3>
                  <p className="text-gray-600 mb-8 max-w-md mx-auto">
                    Feed Instagram tidak dapat dimuat saat ini. Kunjungi profil Instagram kami untuk melihat konten terbaru!
                  </p>
                  <a
                    href="https://instagram.com/bro.do"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-3 px-8 py-4 bg-gradient-to-r from-purple-600 to-pink-600 text-white font-semibold rounded-lg hover:from-purple-700 hover:to-pink-700 transition-all duration-200 shadow-lg hover:shadow-xl transform hover:scale-105"
                  >
                    <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/>
                    </svg>
                    Buka Instagram @bro.do
                  </a>
                </div>
              ) : (
                <>
                  ...
                  {!isJuicerLoaded && (
                    <div className="text-center py-16">
                      <div className="inline-block animate-spin rounded-full h-12 w-12 border-b-2 border-brodo-blue mb-4"></div>
